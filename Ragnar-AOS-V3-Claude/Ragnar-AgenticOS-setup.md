...
- **Their OS** (e.g., `kumi-os`, `alice-os`, `bob-ai-team`) installed as a skill under their chosen name in `~/.claude/skills/<their-name>/` and/or `~/.github/skills/<their-name>/`.
- **An auto-boot phrase** they picked (e.g., "Kumi", "hey Alice") that launches their OS from any project.
...
When invoked, this file makes your AI assistant (Claude Code or GitHub Copilot CLI) do the following, in order:

1. **Ask for a name.** Three short questions: skill slug, display name, wake-up phrase.
2. **Detect the runtime.** Claude Code (`~/.claude/`), GitHub Copilot CLI (`~/.github/`), or both.
...
- If `~/.claude/` exists → install for Claude Code.
...
3. For each detected runtime, write the content to `<runtime>/skills/<OS_NAME>/<relative-path>`:
   - Claude Code: `~/.claude/skills/<OS_NAME>/<relative-path>`
...
- In the destination `commands/<OS_NAME>.md`, replace the phrase `Invoke the \`raos\` skill` with `Invoke the \`<OS_NAME>\` skill`, and the SKILL.md path reference `~/.claude/skills/raos/` with `~/.claude/skills/<OS_NAME>/`.
...
2. Check for `.claude/agents/cli-lead.md` (existing team).
...
Installed <OS_DISPLAY> (blueprint v1.09)
  ...
  Claude Code: <yes / skipped-not-present>
...
|---|---|---|
| `/raos` (no args) | `.claude/agents/` missing | **Bootstrap** a team (see below) |
...
| `/raos reset` | team exists | Confirm with user, then wipe `.claude/agents/` + `tasks.json` |

Detect state by reading CWD: look for `.claude/agents/cli-lead.md` or equivalent orchestrator file. If present, team exists.
...
Use the Agent tool with `subagent_type: Explore` (for research), `subagent_type: Plan` (for challenge), and project specialists from `.claude/agents/` (for synthesis). Run parallel where there are no dependencies.
...
| Symptom | Fix |
|---|---|
| `/raos` doesn't appear as a command | Restart Claude Code session. Skill must be in `~/.claude/skills/raos/`. |
...
| Teammate can't install | `tar -czf raos.tar.gz ~/.claude/skills/raos/` → send file → extract into their `~/.claude/skills/`. No other steps. |
| Stage 3 feels risky | Stay in Stage 2 longer. Build trust in the team. Stage 3 is earned, not configured. |
| `/raos reset` wiped work | It was confirmed. Recover `.claude/agents/` from git. This is why CLAUDE.md + agents should be committed. |
...
Pure Claude Code. No npm. No repo clone. No tmux required. No GitHub Copilot CLI needed. The skill is self-contained in `~/.claude/skills/raos/`. Share it by tarballing the folder.
...
3. Branch on `$ARGUMENTS`:
   ...
   - **Empty + team exists** → show **Status**: list agents in `.claude/agents/`, summarize `tasks.json` (open/in-progress/done counts), last activity time. Offer next actions.
   - **`status`** → same as status above.
   - **`reset`** → confirm with the user ("this will delete `.claude/agents/` and `tasks.json`. git-tracked? type YES to proceed"). Only on explicit YES, wipe.
   - **Any other text + team exists** → treat as an **objective**. Reformat into the shape from [templates/objective.template.md](~/.claude/skills/raos/templates/objective.template.md), confirm with user, then dispatch via the 3-phase pattern in [references/3-phase-execution.md](~/.claude/skills/raos/references/3-phase-execution.md).
...
In Claude Code: multiple Agent tool calls in a single message run concurrently. Use that. The Team Lead should favor breadth over depth whenever the work is parallelizable.
...
- **Project specialists (your own .claude/agents/):** whatever they're scoped to. Parallel if they own disjoint surfaces.
...
If the teammate has tmux and wants to see parallel execution live, they can split the terminal into panes. one per specialist. and tail their logs. Not required. The native task list (`/raos status` → `.claude/tasks.json`) carries the same information.
...
Replace `<org>` with your ADO org name. each org gets its own MCP server (see [ado-mcp-setup](~/.claude/skills/ado-mcp-setup/SKILL.md)).
...
- Adding/removing team members from `.claude/agents/`.
...
- Sub-agents in `.claude/agents/` with clear names and ownership.
...
This project runs on RAOS (Ragnar's Agentic OS). The user speaks to the Team Lead (`.claude/agents/cli-lead.md`). The Team Lead delegates to specialists. All work flows through `.claude/tasks.json`.
...
Pre-built `.claude/agents/` definitions RAOS copies into your project during `/raos` bootstrap, based on the integrations you opt into.
...
Every agent in `.claude/agents/` is context the Team Lead reads. Fewer agents = tighter context = faster decisions.

---

## Customizing an agent

After `/raos` copies an agent into your project's `.claude/agents/`, it's yours. Edit it. Tighten it. Add project-specific MCP tool names. Narrow the bucket lists based on how much you trust the specialist on this codebase.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~RAOS-FILE-START templates/tasks.json.schema
{
  ...
  "description": "Lives at .claude/tasks.json in the project. The Team Lead writes and reads this. /raos status renders it. Atomic writes. read, modify in memory, write whole file.",
  "type": "object",
    ...
    "tasks": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "owner": { "type": "string", "description": "Specialist agent name (file stem in .claude/agents/). 'cli-lead' for orchestration work." },
          "coordinates_with": {
            "type": "array",
            "items": { "type": "string" },
...
2. **Specialist composition**. you decide which of the agents in `.claude/agents/` to dispatch for a given objective, and in what order.
...
3. **Dispatch** in 3 phases:
   ...
   - **Synthesis**. Project specialists (from `.claude/agents/`) execute. You merge. You verify against SUCCESS LOOKS LIKE. If any criterion is unmet, loop back to Synthesis with a targeted fix. do not hand the user a partial result.
...
TRACE="$RUN_DIR/trace.ndjson"
TASKS_JSON="$PROJECT_ROOT/.claude/tasks.json"
  ...
  resume:
    ...
    steps:
      ...
      - name: Resume RAOS run
        run: |
          chmod +x .claude/skills/<OS_NAME>/harness/bin/headless.sh
          .claude/skills/<OS_NAME>/harness/bin/headless.sh "$GITHUB_WORKSPACE" "<RUN_ID>"
...
{
  "//": "RAOS V2 Scaffold — hook registration fragment. The installer merges this into ~/.claude/settings.json. <OS_NAME> is the personalized OS slug; <SKILL_DIR> is the absolute path to the installed skill folder.",
  "hooks": {
    "PreToolUse": [
      {
        "matcher": ".*",
...
The Stop hook also appends an auto checkpoint whenever Claude Code stops. That's why a crashed session still has a recent checkpoint to resume from.
...
| Stop             | on Claude Code stop | writes `checkpoint.json`, appends to `manifest.checkpoints[]`, runs `costs.sh` |
